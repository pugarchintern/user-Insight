...
export default function DashBoard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([contextData, locationsData]) => {
  ...
  // 3. Cascading: Cleaner options dynamically filter based on Selected Company AND Selected Location
  const cleanerOptions = [
    "All",
    ...new Set(
      activities
        .filter((a) => {
          const matchesCompany = selectedCompany === "All" || String(a.company_id) === selectedCompany;
          const matchesLocation = selectedLocation === "All" || a.location?.name === selectedLocation;
          return matchesCompany && matchesLocation;
        })
        .map((a) => a.cleaner?.name)
        .filter(Boolean)
    ),
  ];
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans p-6 md:p-8">
      ...
      <section className="max-w-7xl mx-auto bg-white border border-slate-100 rounded-2xl p-4 shadow-sm mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 flex-1 justify-end">
          ...
          {/* 1. Company ID Select Option (Resets dependent dropdowns on change) */}
            ...
            >
              {companyOptions.map((opt, index) => (
                <option key={index} value={opt}>
                  {opt === "All" ? "All Companies" : `Company ${opt}`}
                </option>
              ))}
            </select>
          </div>
...
//   useEffect(() => {
//     fetch(
//       "https://daily-dash-backend-development.vercel.app/api/ai-insights/context",
//     )
//       .then((res) => res.json())
//       .then((data) => {
...
//   // 2. Cascading: Location options dynamically filter based on Selected Company
//   const locationOptions = [
//     "All",
//     ...new Set(
//       activities
//         .filter((a) => selectedCompany === "All" || String(a.company_id) === selectedCompany)
...
//   const cleanerOptions = [
//     "All",
//     ...new Set(activities.map((a) => a.cleaner?.name).filter(Boolean)),
//   ];
//   const locationOptions = [
//     "All",
//     ...new Set(activities.map((a) => a.location?.name).filter(Boolean)),
//   ];

//   const filteredActivities = activities.filter((activity) => {
//     const matchesCompany =
//       selectedCompany === "All" ||
//       String(activity.company_id) === selectedCompany;
//     const matchesCleaner =
//       selectedCleaner === "All" || activity.cleaner?.name === selectedCleaner;
//     const matchesLocation =
//       selectedLocation === "All" ||
//       activity.location?.name === selectedLocation;
...
//         <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 hover:scale-[1.02] hover:shadow-md transition-all duration-300">
//           <div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl shrink-0">
//             <ClipboardDocumentCheckIcon className="w-6 h-6" />
//           </div>
//           <div>
//             <span className="block text-2xl font-black text-slate-900 leading-none">{summary.totalActivities || 0}</span>
//             <span className="text-[10px] font-bold text-slate-400 mt-1.5 block uppercase tracking-wider">Total Activities</span>
//           </div>
//         </div>
...
//             <span className="text-[10px] font-bold text-slate-400 mt-1.5 block uppercase tracking-wider">Active Cleaners</span>
//           </div>
//         </div>
...
//             <span className="text-[10px] font-bold text-slate-400 mt-1.5 block uppercase tracking-wider">Locations Tracked</span>
//           </div>
//         </div>
...
//             <span className="text-[10px] font-bold text-slate-400 mt-1.5 block uppercase tracking-wider">Registered Groups</span>
//           </div>
//         </div>
//       </section> */}
